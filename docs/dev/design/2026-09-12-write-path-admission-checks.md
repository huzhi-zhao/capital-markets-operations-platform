...
1. **清单用什么格式，放在哪里。** 候选是与建表定义同目录的一份 YAML，或直接在建表定义里加标记。
   **后者少一处不同步的可能，前者便于一次性检视全貌。** 未决。
2. **维护排期字段是写周期还是写触发条件。** 周期简单但与实际堆积速度无关，
   触发条件更准但需要第三层先跑起来。**倾向先写周期，等 B3 有数据后再改。**
3. **静态检查用什么实现。** 规则都是键值匹配和交叉引用，
   一段脚本足够，**但要与将来的数据质量框架保持一致的失败报告格式**。
