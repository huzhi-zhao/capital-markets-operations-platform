...
**本节已于 2026-09-12 提升为 [ADR 0007](adr/0007-external-interactive-scope-reads-gold-only.md)。**
上面留的条件已经成立：异常解释表构成了工作负载基线的一条显式例外，而本决定已被四处引用。
**正文保留不动**，ADR 补的是被否决的第三个选项、"可复现"的判据，以及本决定会在什么条件下
被推翻。
