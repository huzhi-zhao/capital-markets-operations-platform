...
### 3A.1 先决定一件事：第二批应当接在 L-1 后面，而不是另起一段

第一批冻结了 L-1，一条走完的订单生命周期。第二批若独立挑几条分配报文核对，得到的是互不相
连的片段，**而 BO-1 要证明的是一条链能从下单走到结算**。

**因此建议先冻结 A-1：把 L-1 那张已成交的订单分配到若干账户，直到分配被接受。** A-1 必须
以 L-1 的终态为输入，用 L-1 产出的 `OrderID` 与累计成交量，不重新编一笔。这样第二批结束时，
链条从下单连续到分配，而不是多出一段孤立证据。
...
| 编号 | 场景 | 为什么要它 | 状态 |
|---|---|---|---|
| A-1 | L-1 的已成交订单分配到 N 个账户，一次通过 | 打通链条第三段，最小可证 | **冻结候选** |
