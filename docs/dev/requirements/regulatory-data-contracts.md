...
| S-5 | 冲正（`sese.026`） | 与公司行为重述交互，最难的一类。**原 S-4 改号至此** | 待解锁，**依赖重述路径先跑通** |
...
| P1-7 | `FctvIntrBkSttlmDt` 等于 `IntrBkSttlmDt` | 仅 P-1；延迟变体下大于 |
...
| P-2 | 资金腿延迟，券腿正常 | 检验 DVP 两腿脱钩，`FctvIntrBkSttlmDt` 大于 `IntrBkSttlmDt` | 待解锁，与 S-2 配对价值最高 |
| P-3 | 退汇（`pacs.004`） | 制造资金段的反向事实，与重述交互 | 待解锁 |
| P-4 | 账户对账（`camt.053`） | 用日终对账单核对资金段，`ExternalEntryStatus1Code` 四个值全部用上 | 待解锁；BQ-1 的对账故事落在这里 |
