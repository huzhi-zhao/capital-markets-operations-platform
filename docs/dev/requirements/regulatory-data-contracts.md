...
| 第三批 | ISO 20022 结算报文族 | 三种核心报文的标识符、版本与基数已从规范 XSD 核对；S-1 结构已定形；用法规则未读 |
...
**第三批于 2026-09-10 开工。** 证券结算这一支已核对到可用程度，其余三支仍是待验证的起点。
...
**出处是 ISO 20022 注册机构发布的规范 XSD 本身**，不是任何二手说明。取自消息集
**Settlement and Reconciliation**（最后更新 2026-03-17），逐条 schema 均由该站点直接取得。
XSD 是规范性文本，**基数（`minOccurs` / `maxOccurs`）与枚举值不存在解释空间**。

**但 XSD 不是全部，这一点必须写在最前面。** ISO 20022 的业务定义、用法规则（usage rules）与
市场实践写在 **Message Definition Report**（MDR）里，MDR 随消息集整包下载，**本轮尚未读**。

**因此本节的结论分两级，不要混用：**

| 级别 | 内容 | 可靠性 |
|---|---|---|
| 来自 XSD | 报文标识符、版本号、元素基数、枚举取值 | **规范性，MDR 不可能推翻** |
| 待 MDR 校验 | 哪些可选元素在实务中必填、跨元素的用法约束、状态转移的合法次序 | **未知，可能推翻 §4.5 的取值选择** |

**已知 MDR 可能改变什么：** §4.5 给 S-1 选的那些可选字段是否够用、结算失败与挂起之间的合法
转移次序、以及市场基础设施对 `SctiesTxTp` 取值的限制。**S-1 的结构不会变，取值可能变。**
...
| 撮合状态 | `MtchgSts` / `IfrrdMtchgSts` | 三选一：已撮合、未撮合、专有 |
| 结算状态 | `SttlmSts` | 三选一：挂起、失败、专有 |

**三个轴可以同时有值，也可以一个都没有。** 一条不带任何状态的 `sese.024` 是 schema 合法的。
**"状态机"这个词在这里要谨慎使用**，它不是一张状态转移图，是三张。
...
| 步 | 报文 | 方向 | 关键取值 | 之后状态 |
|---|---|---|---|---|
| 1 | `sese.023` | 账户持有方 → 账务服务方 | `TxId` 由种子派生；`SctiesMvmntTp` 按 L-1 的 `Side` 取 `DELI`（卖）或 `RECE`（买）；`Pmt=APMT`；`SttlmDt` 取 `TradeDate` + 1 营业日；`SctiesTxTp=TRAD`；`SttlmQty/Unit` 取该账户的 `AllocQty`；`FinInstrmId/OthrId` 用合成标的代码 | 已发出 |
| 2 | `sese.024` | 账务服务方 → 账户持有方 | `TxId/AcctOwnrTxId` 回引第 1 步；`PrcgSts/AckdAccptd` | 已受理 |
| 3 | `sese.025` | 账务服务方 → 账户持有方 | 回引第 1 步；`SttldAmt` 取该账户的 `AllocNetMoney` | 已结算，S-1 结束 |
...
| 编号 | 场景 | 为什么要它 | 状态 |
|---|---|---|---|
| S-1 | 逐账户结算指令、受理、结算确认，一次通过 | 链条第五段，也是最后一段 | **结构已定形**，取值待 MDR 校验 |
| S-2 | 结算失败后延迟结算 | **直接对应主业务流**，检验同一原因码在结算日前后落在不同状态轴 | 待解锁，价值最高 |
| S-3 | 结算指令被撤销（`sese.020` 与 `sese.027`） | 制造同一 `TxId` 的多版本 | 待解锁 |
| S-4 | 冲正（`sese.026`） | 与公司行为重述交互，最难的一类 | 待解锁 |

### 4.8 第三批还欠什么

- **MDR 未读**，见 §4.1。这是本批最大的未知，**在读完之前不要把 S-1 称作"已冻结"**。
- `camt` 与 `pacs` 属第四批，尚未开始。
- 结算段的业务校验层未写，见 §4.3 末尾。
- **S-1 引入了账务服务方与账户持有方两个新角色**，[合成数据生成规范](data-generation-specification.md)
  §3A 的两层结构里没有它们。是复用客户层还是新增一层，未决。
...
| ISO 20022 | Settlement and Reconciliation 消息集，最后更新 2026-03-17 | [ISO 20022 报文定义目录](https://www.iso20022.org/iso-20022-message-definitions)，`sese` 族规范 XSD 逐条取得；**Message Definition Report 尚未取得** | 2026-09-10 |
| FINTRAC | | | |
