...
| P-5a | 未入账后来被拒 | 日内报告说 `PDNG`，日终对账单里根本没有 | 待解锁，见 §4B.12 末段 |
...
**P5-2 与 P5-4 都是否定式。** 加上 P6-7 与 VA-12，**清单里的否定式断言已有四条**，
**它们共同的特点是：要挡的不是坏数据，是一个看上去合理的写法。**

#### P-5 特意不做的三件事

1. **不做未入账后来被拒的分支。** 那是"日内报告说 `PDNG`、日终对账单里根本没有"，
   **它需要先有 `pacs.002` 的 `RJCT` 与 `camt.052` 的联动**，留给 P-5a。
2. **不做 `FUTR`。** 登记时把 `PDNG` 与 `FUTR` 并列，**但两者的时间轴不同**：
   `PDNG` 是当日待入账，`FUTR` 是未来日期已确定。**`FUTR` 会把场景拉成跨日**，
   与起息日不同于入账日的那条一起做，登记为 P-5b。
