...
**内存**：CMOP 能用的不是 24 GB，是约 7.9 GB。这个数字恰好和默认组合的实测占用 7.6 GB
接近，**意味着再起一套平行的栈几乎不可能**。

**磁盘是硬阻塞。** [平台架构](../platform-architecture.md) §8 给 Gold 的预算是 40 GB，
而这台机器只剩 32 GB。**Gold 按现有预算放不下**，且这还没算 catalog 后端、日志与
Iceberg 元数据。

#### 由此得出的三条，这次有实测支撑

1. **复用不再是可选项，是唯一可行解。** [平台架构](../platform-architecture.md) §10 里
   "是否复用 UOIP 既有 Airflow 实例"这条开放项，现在有证据了：Trino、Airflow、Kafka、
   Flink、Superset、Postgres 全都已在运行，再起一套装不下。§4.2 关于只有一套编排的论证
   因此从设计偏好升级为资源事实。
2. **Catalog 仍必须独立。** §7 的"CMOP 使用独立 REST Catalog，不与 UOIP 共用 Hive
   Metastore"不受影响，那台机器上的 Hive Metastore 属于 UOIP，复用它会把两个项目的表
   命名空间搅在一起。复用计算与编排，不复用元数据。
3. **Gold 的 40 GB 预算必须重做。** 这是三条里唯一的硬阻塞，且它推翻的是容量模型而不是
   选型。要么压缩 Gold，要么扩块存储，要么把 Gold 挪到别处。
