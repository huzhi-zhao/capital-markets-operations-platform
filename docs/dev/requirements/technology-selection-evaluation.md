...
#### 2.6.1 表清单的形态已定，只剩演练待做

**定于 2026-09-12。** §2.6 要求的两样东西里，第一样不需要任何机器就能定下来。

| 项 | 决定 | 理由 |
|---|---|---|
| 内容 | 表名、表位置、最近一次已知 metadata 文件路径、写入方、维护周期 | 前两项是 `register_table` 的输入；第三项把"哪个是最新"从猜测变成有据可查；后两项与准入检查的双写表清单同源，**一份清单不要拆成两份** |
| 格式 | 纯文本，一行一表，字段以制表符分隔 | **必须能在没有任何组件可用时被人读懂**。恢复场景的前提就是 catalog 后端没了，此时依赖某个服务去解析清单是循环依赖 |
| 存放 | 与[工作负载基线](../workload-baseline.md) §5.6 判定不可替代的参考数据同处 | 那批数据已有备份安排，**清单很小，搭车比另建一套便宜** |
| 更新时机 | **建表与删表时同步更新，不定期扫描** | 定期扫描会在两次扫描之间留下窗口，而**窗口期内新建的表恰好是最容易被遗忘的那批** |
| 更新方式 | 由准入检查那一层顺带写出，见[写入路径准入检查](../design/2026-09-12-write-path-admission-checks.md) | 建表路径上已经有一道必过的检查，**再加一个独立的钩子只会多一处可以被绕过的地方** |

**"最近一次已知 metadata 文件路径"这一列是本决定的实质。** §2.6 记的第二个缺口是
"哪个 metadata.json 是最新的没有规范级判定方式"，**清单里记一笔就把这个缺口从
...
- ~~Catalog 后端存储能否从对象存储中的 metadata 文件重建。~~ **规范层面已答，见 §2.6**：
  路径存在，但缺表清单与最新版本判定。**表清单的形态已于 2026-09-12 定案，见 §2.6.1**；
  **重建耗时与最新版本判定的可靠性仍需实测**，绑在部署窗口上。
